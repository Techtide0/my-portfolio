...
interface ProjectLinks { github?: string; live?: string; }
interface Project {
  title: string;
  description: string;
  tags: string[];
        ...
        <main id="about" className="w-full min-h-screen flex flex-col items-center justify-center px-6 py-20">
          <div className="max-w-6xl mx-auto text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-16">
              <button onClick={ctaButtons.primary.onClick} className="primary-button px-6 py-3 text-foreground rounded-lg font-medium text-sm min-w-40">
                {ctaButtons.primary.label}
              </button>
              <button onClick={ctaButtons.secondary.onClick} className="glass-button min-w-40 inter-font text-sm font-medium text-foreground rounded-lg px-6 py-3">
            ...
            {/* ── Projects grid ── */}
            <div id="projects" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 max-w-7xl mx-auto mb-16">
              {projects.map((project, index) => (
                <div key={index} className="glass-card rounded-2xl p-5 text-left flex flex-col">
                  {/* Type badge */}
                  {project.type && (
                    <span className="type-badge mb-3">{project.type}</span>
                  )}

                  {/* Preview image slot */}
                  <div className="project-image rounded-xl h-28 mb-4 flex items-center justify-center overflow-hidden">
                    {project.imageContent}
                  </div>

                  {/* Title */}
                  <h3 className="text-base font-semibold text-card-foreground mb-1 geist-font leading-snug">
                    {project.title}
                  </h3>

                  {/* Subtitle */}
                  {project.subtitle && (
                    <p className="text-xs text-muted-foreground inter-font mb-2 opacity-70">
                      {project.subtitle}
                    </p>
                  )}

                  {/* Description */}
                  <p className="text-muted-foreground text-sm inter-font mb-4 leading-relaxed flex-1">
                    {project.description}
                  </p>
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {project.tags.map(tag => (
                      <span key={tag} className="skill-badge px-2 py-0.5 rounded text-xs text-muted-foreground">
                        {tag}
                      </span>
                    ))}
                  </div>
              ...
              {stats.map((stat, index) => (
                <React.Fragment key={stat.label}>
                  ...
                  {index < stats.length - 1 && (
                    <div className="hidden sm:block w-px h-12 bg-linear-to-b from-transparent via-input to-transparent" />
                  )}
                </React.Fragment>
              ))}
            </div>
